...
def build_report(
    ...
    # De koers uit de runconfiguratie, met de huidige als terugval. Runs die
    # begonnen voordat de koers werd opgeslagen hebben hem niet in hun
    # configuratie; dan bleef de kolom leeg terwijl de bedragen klopten.
    koers = opzet.get("conversion_rate")
    rekenvaluta = opzet.get("account_currency")
    if not koers and conversion:
        koers = conversion.get("rate")
    if not rekenvaluta and conversion:
        rekenvaluta = conversion.get("account")
    rekenvaluta = rekenvaluta or "account"
...
def write_report(
    ...
    target.write_text(
        build_report(db, run_id, gate, tz=tz, conversion=conversion),
        encoding="utf-8",
    )
    return target
